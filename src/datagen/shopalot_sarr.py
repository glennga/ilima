...
def generate_stores(handler, fake_datagen, start_id, end_id):
    for s in range(start_id, end_id):
        ...
        store_categories = [random.choice(product_categories)]
        store = {
            "store_id": store_id,
            "name": store_name,
            "address": {
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate single-element-array ShopALot JSON data.')
    parser.add_argument('dataset', type=str, choices=['user', 'store', 'order', 'product'],
    ...
    filename = command_line_args.filename if command_line_args.filename is not None else {
        ...
        'product': 'ProductsOutput.json'
    }[command_line_args.dataset]
    fp = open(filename, 'w')

    if command_line_args.dataset == 'order':
        generate_products(lambda a: None)

    if command_line_args.dataset in ['user', 'store', 'order']:
        ...
        generate_products(lambda a: _file_handler(fp, a))
